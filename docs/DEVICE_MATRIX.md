...
| VNish, LuxOS | Antminer forks | Standard cgminer `version`/`summary`/`pools` | ✅ Basic (hashrate, shares, uptime, pool) | ❌ | Detected by `version`; temps/fans/power pending capture from real units (their `stats` layout / REST APIs differ). Declines Avalon/BOSer |
