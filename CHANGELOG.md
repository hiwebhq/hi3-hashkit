...
- Wall / TV mode: tiles group by rack (B1-R1) again instead of one group per slot code,
  so the grid-columns setting fills the screen; tiles sort in physical rack order. A new
  "1h hashrate" option draws a last-hour sparkline on every tile, and tile text
  auto-shrinks when a forced column count makes tiles narrow.
